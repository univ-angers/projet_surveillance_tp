...
public class formUtilisateur extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		
		...
		else response.sendRedirect(request.getContextPath()+"/listeUtilisateurs");
	}

	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// Création d'un utilisateur si les conditions sont remplies
	...
	public boolean traiterDonnees(String nom, String prenom, String mail) {
		if(!nom.isEmpty() && !prenom.isEmpty() && !mail.isEmpty()) return true;
		else return false;
	}

	public String traiterMotDePasse(String mdp) {
		if(mdp.length()>=5) return mdp; // Traiter le mot de passe (chiffrer)
		else return "";
	}
}