...
public class DAORegleImpl implements DAORegle {
	...
	DAORegleImpl( DAOFactory d ) {
		daoFactory=d;
	}
	
	/*
	 * Simple méthode utilitaire permettant de faire la correspondance (le mapping)
	 * entre une ligne issue de la table des Rule (un ResultSet) et un bean Regle.
	 */
	...
	public void creer(Regle regle) throws DAOException {
		...
		try {
			...
			if(valeursAutoGenerees.next()) regle.setIdRegle(valeursAutoGenerees.getInt(1)); // Puis initialisation de la propriété id du bean Utilisateur avec sa valeur
	...
	public Regle trouverSt(String typeRegle) throws DAOException {
		...
		try {
			...
			if(resultSet.next()) regle=map(resultSet);
		} catch (SQLException e) {
			throw new DAOException(e);
		} finally {
